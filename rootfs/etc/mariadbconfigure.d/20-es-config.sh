...
sed -i "s/ML_USER:.*$/ML_USER: \"${MLAPIDB_USER}\"/g" /config/zm_secrets.yml
sed -i "s/ML_PASSWORD:.*$/ML_PASSWORD: \"${MLAPIDB_PASS}\"/g" /config/zm_secrets.yml
sed -i "s/ml_enable:.*$/ml_enable: \"yes\"/g" /config/objectconfig.yml
sed -i "s/gateway:.*$/gateway: \"http://${MLAPI_CONTAINER}:${MLAPI_PORT}/api/v1\"/g" /config/objectconfig.yml
